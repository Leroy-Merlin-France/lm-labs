...
public class CreatedSiteActivitiesEventListener extends
		SendMailActivitiesEventListener {

    private static final Log LOG = LogFactory.getLog(CreatedSiteActivitiesEventListener.class);

	@Override
	void sendDocument(DocumentModel doc, DocumentEventContext ctx)
			throws Exception {
		List<String> sendTo = DirectoriesUtils.getDirList(Directories.NOTIFICATION);
        if (sendTo.size() > 0){
			Map<String, Object> infoMap = new HashMap<String, Object>();
	        infoMap.put("document", doc);
	        infoMap.put(NotificationConstants.SUBJECT_TEMPLATE_KEY, "subjectCreatedSite");
	        infoMap.put("sender",  (NuxeoPrincipal)ctx.getPrincipal());
	        infoMap.put(NotificationConstants.TEMPLATE_KEY, "createdSite");
	...
	boolean accept(DocumentModel doc, String eventName)  {
		if (!(DocumentEventTypes.DOCUMENT_CREATED.equals(eventName)) || doc == null) {
            return false;
        }
